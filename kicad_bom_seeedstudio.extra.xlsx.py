...
def parse_kicad_xml(input_file):
    ...
    * other parts are designated with "MPN"
    ...
    for f in root.findall('./components/'):
        ...
        opl, mpn, link, description = None, None, None, None
        if fields is not None:
            for x in fields:
                ...
                if x.attrib['name'].upper() == 'LINK':
                    link = x.text
                if x.attrib['name'].upper() == 'DESCRIPTION':
                    description = x.text
        if opl:
            components[name] = opl
        elif mpn:
        ...
        if description:
            ...
            descriptions[name] = ''
    return components, links, descriptions, missing

def write_bom_seeed(output_file_slug, components, links, descriptions):
    ...
    for c in components:
        if components[c] not in parts:
            ...
            partsdesc[components[c]] = []
        parts[components[c]] += [c]
        partslink[components[c]] = links[c]
        partsdesc[components[c]] = descriptions[c]

    field_names = ['Designator', 'MPN/Seeed SKU', 'Qty', 'Description', 'Link']
    field_sizes = {}
    
    workbook = xlsxwriter.Workbook("{}.extra.xlsx".format(output_file_slug))
    ...
    for p in sorted(parts.keys()):
        ...
        worksheet.write_string( row, 3, partsdesc[p], cellformats[3])
        worksheet.write_url( row, 4, partslink[p], cellformats[4])

        if len(designators) > field_sizes[0]:
            field_sizes[0] = len(designators)
        if len(p) > field_sizes[1]:
            field_sizes[1] = len(p)
        if len(partsdesc[p]) > field_sizes[3]:
            field_sizes[3] = len(partsdesc[p])
        if len(partslink[p]) > field_sizes[4]:
            field_sizes[4] = len(partslink[p])
...
if __name__ == "__main__":
    ...
    components, links, descriptions, missing = parse_kicad_xml(input_file)
    write_bom_seeed(output_file, components, links, descriptions)
    if len(missing) > 0:
